...
| `tests/triggerAdPoints.py` | **Interstitial trigger/cooldown coverage — ONLINE only, not in `run_all.py`.** Verifies the five observed rules: under 30s on the table then Options has no ad; over 30s then Help has an ad; over 30s then Complete Game has an ad on Victory; New within 30s of closing that Victory ad has no ad; and New after more than 30s on Victory has an ad. Entry ads are valid cooldown-expiry events and are closed before the table clock starts; one inherited-cooldown ad on the first Options attempt gets one clean retry. The shared closer always handles StoreKit X before the ad's own X, saving `log/ad_unmatched.png` before an unmatched-X failure. `--part cooldown` and `--part victory` run the groups independently. Never cold-launches. |
...
- **Ads themselves are now a tested axis, and they are hard to escape.** Measured
  ...
  the main menu; after more than 30s on that menu, the next resume or new deal
  can show another ad. Treat that menu dwell as its own trigger/cooldown path.
