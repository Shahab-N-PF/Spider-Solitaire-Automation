...
def compare_one(name, ignore, common=None):
    ...
    if name == "Play.png":
        result["cards"] = card_size.compare(
            cv2.imread(os.path.join(BASE_DIR, name)),
            cv2.imread(os.path.join(UNITY_DIR, name)),
            PLAY_CARDS)
    return result
...
def _write_diff(name, changed, mask, frac):
    ...
    crop = None
    if name == "Play.png":
        base, annotated, crop = card_size.mark(base, annotated, cur, PLAY_CARDS)
    combo = np.hstack([
